...
for i in eachindex(Ns_span) # for Ns
    ...
    for j_num = 1:3
        ...
        for k in 1:num_try # for R^* and S^*
            ...
            Fractions[i,j_num,k,1] = mean(fstates[:,1]) # surviving
            Fractions[i,j_num,k,2] = mean(fstates[:,2]) # fluc
            Fractions[i,j_num,k,3] = mean(fstates[:,3]) # alternative SS
            ...
            if size(ufs)[2] == num_init
                ...
                if length(PCA[PCA .> 0.05*maximum(PCA)]) <= 1
                    Fractions[i,j_num,k,3] = 0.0 # alternative stable states
                end
            end
...
# save
myfilename = "Dis"*string(disturb)*"SimuFractions"*string(task_id)*".mat"
