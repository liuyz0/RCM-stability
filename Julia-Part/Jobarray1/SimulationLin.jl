...
tick()
for i in 1:length(Ns_span) # for Ns
Ns = Ns_span[i]
...
para = (Ns,Nr,G,C,l,kappa,δ) #(Ns,Nr,G,C,g,δ) 
...
stop = 0
while stop == 0
    prob = ODEProblem(ResConLin!,u0,tspan,para)
    #VCABM3() or Tsit5()
...
# save
myfilename = "SimuFractionsLin"*string(task_id)*".mat"
